...
def main():
    ...
    print("Training with Complete WESAD Dataset (No Baseline Training)")
    print("="*80)
    ...
    for subject_id, data in all_data:
        ...
        # Skip baseline calculation - use default baseline
        print(f"    ⏭️  Skipping baseline training for S{subject_id}")
        baseline = {
            'heart_rate': 75.0,  # Default baseline values
            'hrv': 0.5,
            'eda': 5.0,
            'breathing_rate': 16.0,
            'tremor': {'variance': 0.0, 'tremor_power': 0.0},
            'skin_temp': {'mean': 36.5, 'std': 0.3}
        }
    ...
    print(f"\n✅ Medical Dataset Prepared (No Baseline Training):")
    print(f"  📊 Total samples: {X.shape[0]}")
    print(f"  🎯 Total features: {X.shape[1]}")
    print(f"  📈 Class distribution: {np.bincount(y)}")
    print(f"  ⏭️  Baseline training: SKIPPED (using default values)")
    ...
    print("⏭️  Baseline training was SKIPPED - using default baseline values")
